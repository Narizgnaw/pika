...
func (h *WebHandler) ServeAdminAsset(c *echo.Context) error {
	return serveStaticFile(c, filepath.Join(assets.WebDir(), "admin", "assets"), c.Param("*"), true)
}
...
func (h *WebHandler) ServeThemeAsset(c *echo.Context) error {
	...
	return serveStaticFile(c, dist, c.Param("*"), true)
}
...
func (h *WebHandler) serveAdminIndex(c *echo.Context) error {
	path := filepath.Join(assets.WebDir(), "admin", "index.html")
